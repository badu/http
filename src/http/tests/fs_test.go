...
func TestServeFileDotDot(t *testing.T) {
	tests := []struct {
		req        string
		wantStatus int
	}{
	...
	for _, tt := range tests {
		...
		rec := th.NewRecorder()
		filetransport.ServeFile(rec, req, "testdata/file")
...
func TestFSRedirect(t *testing.T) {
	...
	defer afterTest(t)
	ts := th.NewServer(StripPrefix("/test", filetransport.FileServer(filetransport.Dir("."))))
	defer ts.Close()

	for _, data := range fsRedirectTestData {
		res, err := cli.Get(ts.URL + data.original)
		if err != nil {
			t.Fatal(err)
		}
		res.Body.Close()
...
func TestFileServerCleans(t *testing.T) {
	...
	fs := filetransport.FileServer(&testFileSystem{func(name string) (filetransport.File, error) {
		ch <- name
...
func TestFileServerEscapesNames(t *testing.T) {
	...
	ts := th.NewServer(filetransport.FileServer(&fs))
	defer ts.Close()
	for i, test := range tests {
		testUrl := fmt.Sprintf("%s/%d", ts.URL, i)
		res, err := cli.Get(testUrl)
		if err != nil {
...
func TestFileServerImplicitLeadingSlash(t *testing.T) {
	...
	ts := th.NewServer(StripPrefix("/bar/", filetransport.FileServer(filetransport.Dir(tempDir))))
	defer ts.Close()
	get := func(suffix string) string {
		res, err := cli.Get(ts.URL + suffix)
		if err != nil {
...
func TestDirJoin(t *testing.T) {
	...
	test := func(d filetransport.Dir, name string) {
		f, err := d.Open(name)
		if err != nil {
	...
	test(filetransport.Dir("/etc/"), "/hosts")
	test(filetransport.Dir("/etc/"), "hosts")
	test(filetransport.Dir("/etc/"), "../../../../hosts")
	test(filetransport.Dir("/etc"), "/hosts")
	test(filetransport.Dir("/etc"), "hosts")
...
func TestServeFileContentType(t *testing.T) {
	...
	get := func(override string, want []string) {
		resp, err := cli.Get(ts.URL + "?override=" + override)
		if err != nil {
			t.Fatal(err)
		}
...
func TestServeFileMimeType(t *testing.T) {
	defer afterTest(t)
	ts := th.NewServer(HandlerFunc(func(w ResponseWriter, r *Request) {
		filetransport.ServeFile(w, r, "testdata/style.css")
	}))
	defer ts.Close()
...
func TestServeFileFromCWD(t *testing.T) {
	defer afterTest(t)
	ts := th.NewServer(HandlerFunc(func(w ResponseWriter, r *Request) {
		filetransport.ServeFile(w, r, "fs_test.go")
	}))
	defer ts.Close()
	r, err := cli.Get(ts.URL)
	if err != nil {
		t.Fatal(err)
	}
	r.Body.Close()
...
func TestServeDirWithoutTrailingSlash(t *testing.T) {
	...
	defer afterTest(t)
	ts := th.NewServer(HandlerFunc(func(w ResponseWriter, r *Request) {
		filetransport.ServeFile(w, r, ".")
	}))
	defer ts.Close()
	r, err := cli.Get(ts.URL + "/testdata")
	if err != nil {
		t.Fatal(err)
	}
	r.Body.Close()
...
func TestServeIndexHtml(t *testing.T) {
	...
	ts := th.NewServer(filetransport.FileServer(filetransport.Dir(".")))
	defer ts.Close()
...
func TestServerFileStatError(t *testing.T) {
	...
	name := "file.txt"
	fs := issue12991FS{}
	filetransport.ExportServeFile(rec, r, fs, name, false)
...
func TestLinuxSendfile(t *testing.T) {
	...
	res, err := cli.Get(fmt.Sprintf("http://%s/", ln.Addr()))
	if err != nil {
...
func TestLinuxSendfileChild(*testing.T) {
	...
	srvMx.Handle("/", filetransport.FileServer(filetransport.Dir("testdata")))
	srvMx.HandleFunc("/quit", func(ResponseWriter, *Request) {
		os.Exit(0)
	})
...
func TestFileServerNotDirError(t *testing.T) {
	defer afterTest(t)
	ts := th.NewServer(filetransport.FileServer(filetransport.Dir("testdata")))
	defer ts.Close()

	res, err := cli.Get(ts.URL + "/index.html/not-a-file")
	if err != nil {
		t.Fatal(err)
	}
	res.Body.Close()
	...
	test := func(name string, dir filetransport.Dir) {
	...
	test("RelativePath", filetransport.Dir("testdata"))
	test("AbsolutePath", filetransport.Dir(absPath))
}
...
func TestFileServerCleanPath(t *testing.T) {
	...
	for _, tt := range tests {
		...
		filetransport.FileServer(fileServerCleanPathDir{&log}).ServeHTTP(rr, req)
...
func Test_scanETag(t *testing.T) {
	...
	for _, test := range tests {
		etag, remain := filetransport.ExportScanETag(test.in)
...
func TestFileTransport(t *testing.T) {
	...
	tr := &Transport{}
	tr.RegisterProtocol("file", filetransport.NewFileTransport(filetransport.Dir(dname)))
